...
------------------------------------------------------------------------------
section vec2DToMatrix

variables {m n : ℕ}
variables (v : vector (vector ℂ n) m)

@[simp]
lemma vec2DToMatrix_apply (i : fin m) (j : fin n)
    : vec2DToMatrix v i j = (v.nth i).nth j
:= begin
    refl,
end

@[simp]
lemma vec2DToMatrix_adjoint_apply (i : fin m) (j : fin n)
    : (vec2DToMatrix v).adjoint j i = ((v.nth i).nth j)†
:= begin
    refl,
end

lemma vec2DToMatrix_unit (l : vector (vector ℂ 1) n)
    : dot_product (λ i, ((l.nth i).head)†) (λ i, (l.nth i).head) = 1
    → ((vec2DToMatrix l)†) ⬝ (vec2DToMatrix l) = 1
:= begin
    unfold matrix.mul,
    intros h,
    apply matrix.ext, intros i j, simp,
